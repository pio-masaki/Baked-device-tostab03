...
static void select_devices(struct audio_device *adev)
{
    ...
    docked_on = adev->devices & AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET | AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET;
    main_mic_on = adev->devices & AUDIO_DEVICE_IN_BUILTIN_MIC;

    ALOGV("hp=%c speaker=%c dock=%c main-mic=%c, hdmi=%c, isRecording=%c", headphone_on ? 'y' : 'n',
          speaker_on ? 'y' : 'n', docked_on ? 'y' : 'n', main_mic_on ? 'y' : 'n', hdmi_on ? 'y' : 'n', isRecording ? 'y' : 'n');
